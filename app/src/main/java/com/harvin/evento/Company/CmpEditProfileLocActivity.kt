...
import com.harvin.evento.databinding.ActivityCmpEditProfileLocBinding
import java.io.IOException
import java.util.*

class CmpEditProfileLocActivity : AppCompatActivity() {
    private lateinit var binding: ActivityCmpEditProfileLocBinding
    var databaseCmp: DatabaseReference? = null
    var fusedLocationProviderClient: FusedLocationProviderClient? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding=ActivityCmpEditProfileLocBinding.inflate(layoutInflater)
        setContentView(binding.root)

        databaseCmp = FirebaseDatabase.getInstance().getReference("company")
        binding.cmpLocReg.setText(""+intent.getStringExtra("location"))
        ...
        binding.cmpLocReg.setOnClickListener {
            ...
            val intent = Autocomplete.IntentBuilder(AutocompleteActivityMode.OVERLAY, fields)
                .build(this@CmpEditProfileLocActivity)
    ...
    private fun getCurrentLoc(){
        if (ActivityCompat.checkSelfPermission(this@CmpEditProfileLocActivity, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
            fusedLocationProviderClient!!.lastLocation.addOnCompleteListener { task ->
                val location = task.result
                if (location != null) {
                    val geocoder = Geocoder(this@CmpEditProfileLocActivity, Locale.getDefault())
                    try {
            ...
            ActivityCompat.requestPermissions(this@CmpEditProfileLocActivity, arrayOf(Manifest.permission.ACCESS_FINE_LOCATION), 44)
        }
    }
