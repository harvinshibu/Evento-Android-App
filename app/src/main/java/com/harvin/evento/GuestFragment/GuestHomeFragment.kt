...
class GuestHomeFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.gstHomeRecycler.setHasFixedSize(true)
        binding.gstHomeRecycler.layoutManager= LinearLayoutManager(context)
        cmpArrayList= arrayListOf()
        databaseCmp = FirebaseDatabase.getInstance().getReference("company")
        loadDataFromDatabase()
        Log.d("Errror----",cmpArrayList.toString())

        binding.gstFilter.setOnClickListener {
        ...
        if(binding.searchGst !=null){
            binding.searchGst.addTextChangedListener(object : TextWatcher {
    ...
    private fun search(string: String) {
        ...
        for (objectList in cmpArrayList!!) {
            if (objectList.cname.toString().lowercase(Locale.getDefault()).contains(string.lowercase(
                    Locale.getDefault()))) {
                searchList.add(objectList)
            }
        }
        homeAdapter = RecyclerUserHomeAdapter(requireContext(), searchList)
        binding.gstHomeRecycler.adapter = homeAdapter
    }

    private fun filter(string: String) {
        val filterList: ArrayList<CompanyModel> = arrayListOf()
        for (objectList in cmpArrayList!!) {
            if (objectList.service.toString().lowercase(Locale.getDefault()).contains(string.lowercase(
                    Locale.getDefault()))) {
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                try{
                    if (snapshot.exists()){
                        ...
                        binding.gstHomeRecycler.setAdapter(homeAdapter)
                    }
